...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \draw[->] (H) -- ++(1.7, 0) |- (fb) -| node[pos=0.85, above, xshift=2.5mm] {\small$-$} (sum);
...
\end{figure}
		...
		\subsection{Distúrbio no domínio z}
			...
			\begin{equation}
				\left.
				\begin{array}{c}
					\displaystyle C_d(z) = -\frac{G_d(z)}{G_c(z) \cdot H(z)} \cdot \frac{G_c(z) \cdot H(z)}{1 + G_c(z) \cdot H(z)} = \frac{G_d(z)}{1 + G_c(z) \cdot H(z)} \cdot D(z) \\[20pt]
					
					\displaystyle G_d(z) = Z\left[\frac{2,5}{s+0,5} \cdot 0,04\right] = 2,5 \cdot 0,04 \cdot Z\left[\frac{1}{s+0,5} \right] = \frac{0,1z}{z-0,741}  \\[20pt]
					
					\displaystyle C_d(z) = -\frac{0,1z}{z-0,741} \cdot \frac{z-0,741}{z-0,741 + G_c\cdot 0,041} \cdot  D(z) \rightarrow \\[20pt]
					
					\displaystyle \rightarrow C_d(z) = - \frac{0,1z}{z-0,741 + G_c\cdot 0,041} \cdot D(z)
...
Admitindo $T = 0,1$ s, $Gc(z) = 1,0$ e $K=2,4$ é necessário inicialmente realizar a conversão do controlador junto com os conversores para o domínio s, para isso é utilizado o segurador de ordem zero.
	...
	\begin{figure}[H]
	    ...
	    \begin{tikzpicture}[
	        ...
	        \node (sensor) [block, right=of red] {\small $0,07$};	        
	        
	        % Nós de entrada e saída
	        \node (angleIn) [left=of sum] {\small$\theta_c$};
	        \node (angleOut) [right=of sensor] {\small$\theta_c$};
	        ...
	        \draw[->] (sensor) -- (angleOut);
	        
	        \node (fb) [coordinate, below=of Gs] {};
			\draw[->] (sensor) -- ++(1, 0) |- (fb) -| node[pos=0.85, above, xshift=2.5mm] {\small$-$} (sum) ;
	    \end{tikzpicture}
	...
	\begin{equation}
		\left.
		\begin{array}{c}
			\displaystyle G_{c}(s) = \frac{20 \cdot 2,4 \cdot 200 \cdot 0,07}{(s+20) \cdot (0,5s^2+s)\cdot 100} = \frac{6,72}{0,5s^3 + 11s^2 + 20s}
		\end{array}
		\right.
		\quad 
	\end{equation}
	...
	\begin{equation}
		\left.
		\begin{array}{c}
			\displaystyle G(s) = \frac{\displaystyle \frac{6,72}{0,5s^3 + 11s^2 + 20s}}{1 + \displaystyle \frac{6,72}{0,5s^3 + 11s^2 + 20s}} = \frac{13,44}{s^3 + 22s^2 +40s + 13,44} \rightarrow \\[30pt]
			
			 \rightarrow  \displaystyle G(s) = \frac{0.037}{s+20,04} + \frac{0.634}{s+0,44} - \frac{0.671}{s+1,52}
		\end{array}
		\right.
		\quad 
	\end{equation}
